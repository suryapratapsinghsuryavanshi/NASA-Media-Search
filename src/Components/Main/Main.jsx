...
export default class Main extends React.Component {
    ...
    componentDidMount(){
        axios.get(`https://api.nasa.gov/planetary/apod?api_key=${this.state.API_KEY}`)
        .then(data => {
            this.setState({
    ...
    render(){
        if(this.state.error === null){
            return(
                <React.Fragment>
                    <h2 className="message" data-testid="loading-text">Loading.....</h2>
                </React.Fragment>
            )
                    ...
                    <React.Fragment>
                        <main>
                            <div className="top_container">
                                <h3 className="image_title" data-testid="image-title">{this.state.request_data.title}</h3>
                            ...
                            <div className="down_container">
                                <div className="image_container">
                                    <img data-testid="image-src" src={this.state.request_data.hdurl} alt="pic_of_the_day" className="pic_of_the_day" />
                                </div>
                                <div className="dec_container">
                                    <p className="image_dec" data-testid="image-dec">{this.state.request_data.explanation}</p>
                                    <p className="image_date" data-testid="image-date">{this.state.request_data.date}</p>
                                    <p className="image_copyright" data-testid="copyright-text">&copy; {this.state.request_data.copyright}</p>
                                </div>
                            </div>
                        </main>
                    </React.Fragment>
            ...
            return(
                <React.Fragment>
                    <h2 className="message" data-testid="error-text">Please Check API Service.</h2>
                </React.Fragment>
            )
        }
    }
